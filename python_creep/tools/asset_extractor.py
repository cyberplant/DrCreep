    ...
    for i in range(256):
        ...
        for y in range(height):
            for x_block in range(width_blocks):
                if data_ptr >= len(sprite_data): continue
                byte = sprite_data[data_ptr]; data_ptr += 1
                
                # Unconditional Multicolor (2-bit) - Logic from Turn 18
                for p in range(4):
                    shift = (3 - p) * 2
                    ...
                    if bits != 0:
                        ...
                        if i <= 5 or (46 <= i <= 51) or (75 <= i <= 80) or (132 <= i <= 138):
                            if bits == 0x01: color = C64_PALETTE[10] + (255,)
                            elif bits == 0x02: color = C64_PALETTE[1] + (255,)
                            elif bits == 0x03: color = C64_PALETTE[13] + (255,)
                        if px < width_px:
                            pixels[px, y] = color
                            pixels[px + 1, y] = color

        # Flip characters (Player, Climbing, Mummy, Frankie)
        if i <= 5 or (46 <= i <= 51) or (75 <= i <= 80) or (132 <= i <= 138):
...
def extract_tiles(char_rom, output_path):
    with open(char_rom, "rb") as f: data = f.read()
    ...
    for i in range(num_tiles):
        ...
        for y in range(8):
            byte = tile_data[y]
            for x in range(8):
                if (byte >> (7 - x)) & 1: pixels[tx + x, ty + y] = (255, 255, 255, 255)
